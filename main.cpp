...
// Environments are structures that map names to values
// They are paired with expressions to give meanings to free variables

// EmptyEnv is the empty environment.
struct EmptyEnv;

// Bindings<Name, Value, Env> is a type that encodes the environment Env
...
// We use Eval  & Apply-style interpreter [SCIP]

// Eval <Exp, Env> :: result is the value of expression Exp ins environment Env.
...
// Applications apply the value of the function expression to the value of the argument expression
...
// Transition the body of the lambda term inside the Closure
